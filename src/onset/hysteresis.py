...
class FiberGraph(object):
    ...
    def draw_graph(
        self, G, node_color="blue", with_labels=False, edge_color="black"
    ):
        ...
        with open(weight_file, "r") as fob:
            for line in fob:
                ...
                if (
                    ...
                    if edge not in link_graph.nodes():
                        logger.error(
                            "edge {} not in graph {}.".format(
                                edge, link_graph.nodes()
                            )
                        )
                        raise KeyError
    ...
    def _init_link_graph(self):
        ...
        for edge in G.edges():
            ...
            longitude = (
                G.nodes()[u]["Longitude"] + G.nodes[v]["Longitude"]
            ) / 2.0
        ...
        for m in H.nodes():
            ...
            for n in H.nodes():
                ...
                if (
                    m_first in n or m_last in n
                ):  # must be able to walk from M to N
                    H.add_edge(m, n)
    ...
    def draw_graphs(self, FIGURES_DIR=None, link_graph=False):
        ...
        if link_graph:
            if self.link_graph is None:
                raise (
                    "Link graph undefined. First call FiberGraph._init_link_graph"
                )
            link_graph = self.link_graph
            link_graph_colors = list(
                nx.get_node_attributes(link_graph, "color").values()
            )
            self.draw_graph(
                link_graph,
                node_color=link_graph_colors,
                with_labels=False,
                edge_color="white",
            )
            ...
            self.draw_graph(
                link_graph, node_color=link_graph_colors, edge_color="white"
            )
    ...
    def simulate_random_starts_and_failures(
        self, steps=10, kind="random", alpha=0.1, beta=0.2
    ):
        # steps = 100
        # kind = 'random'
        # alpha = 0.1
        ...
        out_folder = path.join(
            "figures",
            "node_{}_steps_{}_class_{}_alpha_{}_beta_{}".format(
                len(self.G.nodes()), steps, kind, alpha, beta
            ),
        )
        ...
        for t in range(steps):
            if t == 0:
                self.update_health(
                    t, out_folder, alpha=alpha, beta=beta, start_clean=True
                )
            else:
                self.update_health(
                    t, out_folder, alpha=alpha, beta=beta, start_clean=False
                )
