...
def main():
    ...
    if experiment == "ANS_smc10":
        ...
        CONGESTION = (
            "/home/matt/ripple/simulator/logs/ANS_1Tbps_ripple/link_util.csv"
        )
        REROUTED_PATHS = "/home/matt/network_stability_sim/data/paths/ANS_Rerouted_source_link_only.json"
        NEW_LINK_MAX_HOPS = 3
        N_TARGET_LINKS = 10
        MAX_LINK_UTIL = 0.95
        SOURCE_LINK_ADDITION = (
            False  # new links start from the target link's source node
        )
        REPEAT = False
    ...
    if experiment == "grid_3_smc10":
        ...
        CONGESTION = (
            "/home/matt/ripple/simulator/logs/grid_3_ripple/link_util.csv"
        )
    ...
    if experiment == "linear_10_smc10":
        ...
        CONGESTION = (
            "/home/matt/ripple/simulator/logs/linear_10_ripple/link_util.csv"
        )
    ...
    if experiment == "whisker_3_2_smc10":
        ...
        experiment == "ANS_amc10"
    ):  # any link, multihop, congestion constrained, 10 link
        TOPO = "/home/matt/ripple/simulator/topologies/ANS/ANS_1-20.gml"
    ...
    time_t = 6.0
    congestion_at_time_t = (
        congestion_df.loc[congestion_df["time"] == 6.0]
        .drop("time", axis=1)
        .stack()
    ...
    for i, target_link in enumerate(target_links):
        ...
        for tc in traffic:
            ...
            if (
                target_link[0] in traffic_path
                and target_link[1] in traffic_path
            ):
                ...
                if 1 == second - first:  # Target link is on this path.
                    if NEW_LINK_MAX_HOPS <= 3:
                        prefixed_subpath = tuple(
                            traffic_path[first - 1 : second + 1]
                        )
                        postfixed_subpath = tuple(
                            traffic_path[first : second + 2]
                        )
                        if (
                            len(prefixed_subpath) == 3
                            and not SOURCE_LINK_ADDITION
                        ):
                            subpaths[prefixed_subpath] += traffic[tc][
                                "flow_rate"
                            ]
                        if len(postfixed_subpath) == 3:
                            subpaths[postfixed_subpath] += traffic[tc][
                                "flow_rate"
                            ]

                    if NEW_LINK_MAX_HOPS == 3:
                        prefixed_subpath = tuple(
                            traffic_path[first - 2 : second + 1]
                        )
                        postfixed_subpath = tuple(
                            traffic_path[first : second + 3]
                        )
                        if (
                            len(prefixed_subpath) == 4
                            and not SOURCE_LINK_ADDITION
                        ):
                            subpaths[prefixed_subpath] += traffic[tc][
                                "flow_rate"
        ...
        while subpaths:
            ...
            candidate_link = (
                most_heavily_shared_subpath[0],
                most_heavily_shared_subpath[-1],
            )
        ...
        if len(tc_bypass_links) >= 1:
            ...
            for bypass_link in tc_bypass_links:
                ...
                # if link has insufficient bandwidth,
                    ...
                    bypass_link_cap > 0
                    and bypass_link_cap < traffic[tc]["flow_rate"]
                ):
                    # find what fraction of demand can be allocated to the new link.
                    fraction_available = (
                        bypass_link_cap / traffic[tc]["flow_rate"]
                    )
                    fraction_leftover = 1 - fraction_available
                    demand_allocated = (
                        fraction_available * traffic[tc]["flow_rate"]
                    )
                    ...
                    rerouted_tcs[tc]["paths"]["path1"]["nhop"] = (
                        len(traffic[tc]["hops"]) - 1
                    )
                    rerouted_tcs[tc]["paths"]["path1"]["hops"] = traffic[tc][
                        "hops"
                    ][:]
    ...
    with open(REROUTED_PATHS, "w") as fob:
        json.dump(
            {
                "new_link": new_link_dict,
                "tcs": rerouted_tcs,
                "ntc": len(rerouted_tcs),
            },
            fob,
            indent=4,
        )
