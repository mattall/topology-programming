...
class Attacker():
    ...
    def make_coremelt_attack_matrix(self, n_hosts: int, n_targets: int, strength: int, max_link_util=inf, target_edges=None, save_matrix=True):
        ...
        if target_edges is not None and len(target_edges) < n_targets:
            ...
            rand_targets = np.random.choice(range(len(possible_targets)), missing_targets, replace=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        traffic_str = "200E9"
        n_targets = 2
        print("USAGE: python {0} network topology_file volume number_targets\n\nExample: python {0} ANS data/graphs/gml/ANS.gml 200E9 1".format(argv[0]))
        # exit()
        # netwrok = 'zayo'
    ...
    if path_f != path_file:
        print(f"Graph was not connected. Paths written to {path_f} instead of {path_file}")
        path_file = path_f
    ...
    assert len(attacker.edge_flows.keys()) == 2 * len(edges), "Error number of edges ill defined"
    n_edges = len(edges)
    continuous_series = np.tile(base_matrix, (2 * n_edges, 1)) # used to evaluate ECMP and MCF without ONSET.
    meta_str = ""
    for i in range(n_edges*2):
        ...
        attack_matrix, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, n_targets, attack_traffic, 100E9, [target_edge], save_matrix=False)
        if len(attacked_edges) > 1:
            meta_str += ("{};{}\n".format(i+1, str(target_edge).replace(" ", "")))
        else: 
            target_edges_str = str(target_edge).replace(" ","").replace("]","").replace("[","")
    ...
    with open("./data/traffic/{}_flash_crowd_meta.csv".format(network), 'w') as fob:
    ...
    if 0: # Coremelt One Link, Both directions Link!
        ...
        continuous_series = np.tile(base_matrix, (2, 1)) # used to evaluate ECMP and MCF without ONSET.
        target_edge = attacker.most_used_edge


        attack_matrix1, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, 1, attack_traffic, 100E9, [target_edge], save_matrix=False)
        continuous_series[0] += attack_matrix1
            
        attack_matrix2, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, 1, attack_traffic, 100E9, [list(reversed(target_edge))], save_matrix=False)
        continuous_series[1] += attack_matrix2
        
        outfile = SCRIPT_HOME + "/data/traffic/{}_coremelt_single_link_{:.2e}.txt".format(network, attack_traffic)
        # print("Saving traffic matrix to: {}".format(outfile))
        np.savetxt(outfile, continuous_series.astype(int), fmt='%i')
    ...
    if 0: # Coremelt Every Link Version2!
        ...
        for i in range(n_edges*2):
            if i % 2 == 0:
                ...
                target_edge = tuple(reversed(edges[i//2]))
            attack_matrix, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, 1, attack_traffic, 100E9, [target_edge], save_matrix=False)
            meta_str += ("{};{}\n".format(i+1, str(target_edge).replace(" ", "")))
    ...
    if 0: # Coremelt Every Link!
        ...
        for i, edge in enumerate(attacker.edge_flows.keys()):
            ...
            attack_matrix, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, 1, attack_traffic, 100E9, [edge], save_matrix=False)
