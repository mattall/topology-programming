...
def post_process(test: str, result_ids: list, networks: list):
    ...
    for net in networks:
        ...
        for res_id in result_ids:
            ...
            if (
                res_id == result_ids[0]
            ):  # only need to do this path prep business once. It is the same on each iteration.
                ...
                base_metric_paths = [
                    path.join(data_root, iteration_tag, base_tag, m_tag)
                    for m_tag in metric_tags
                ]

                base_metric_values = [
                    read_result_val(bmp) for bmp in base_metric_paths
                ]

                circuit_regex = re.compile("^[0-9]+_[0-9]+$")
                circuit_tags = [
                    tag
                    for tag in listdir(path.join(data_root, iteration_tag))
                    if circuit_regex.match(tag)
                ]
            ...
            for circuit_tag in circuit_tags:
                circuit_metric_paths = [
                    path.join(data_root, iteration_tag, circuit_tag, m_tag)
                    for m_tag in metric_tags
                ]

                circuit_metric_values = [
                    read_result_val(cmp) for cmp in circuit_metric_paths
                ]

                circuit_metric_percent_difference = [
                    calc_percent_diff(base_val, circuit_val)
                    for base_val, circuit_val in zip(
                        base_metric_values, circuit_metric_values
                    )
                ]
            ...
            plot_points(
                ...
                path.join(
                    plot_dir,
                    "{}_avg_congestion_difference_vs_paths_difference".format(
                        i
                    ),
                ),
            )
            ...
            plot_points(
                ...
                path.join(
                    plot_dir,
                    "{}_max_congestion_difference_vs_paths_difference".format(
                        i
                    ),
                ),
            )
            ...
            plot_points(
                ...
                path.join(
                    plot_dir,
                    "{}_90pctl_congestion_difference_vs_paths_difference".format(
                        i
                    ),
                ),
            )
        ...
        congestion_multi_cdf(
            congestion_difference_avg,
            "$\Delta$ Avg. Congestion",
            net,
            path.join(
                plot_dir, "{}_congestion_difference_average".format(net)
            ),
        )
        congestion_multi_cdf(
            congestion_avg,
            "Avg. Congestion",
            net,
            path.join(plot_dir, "congestion_average"),
        )
        congestion_multi_cdf(
            congestion_difference_90pct,
            "$\Delta$ 90th Pctl. Congestion",
            net,
            path.join(plot_dir, "{}_congestion_difference_90pct".format(net)),
        )
        congestion_multi_cdf(
            congestion_90pct,
            "90th Pctl. Congestion",
            net,
            path.join(plot_dir, "congestion_90pct"),
        )
        congestion_multi_cdf(
            congestion_difference_max,
            "$\Delta$ Max. Congestion",
            net,
            path.join(plot_dir, "{}_congestion_difference_max".format(net)),
        )
        congestion_multi_cdf(
            congestion_max,
            "Max. Congestion",
            net,
            path.join(plot_dir, "congestion_max"),
        )

        if (
            len(congestion_max) == 2
            and len(congestion_90pct) == 2
            and len(congestion_avg) == 2
        ):
            new_d = {
                "Max": congestion_max[10],
                "90th Percentile": congestion_90pct[10],
                "Mean": congestion_avg[10],
            }
            org_d = {
                "Max": congestion_max[0],
...
def post_proc_timeseries(
    ...
    iter_folder = [
        "{}_{}-{}".format(topology, i, iterations)
        for i in range(1, iterations + 1)
    ]
    measurement = [
    ...
    for m in measurement:
        ...
        for itf in iter_folder:
            print(itf)
            experiment_file = [
                path.join(exp, itf, m + ".dat") for exp in experiments
            ]
        ...
        plots_dir = path.join(
            ...
            "data",
            "results",
            "{}_compare_{}_{}_{}".format(
                topology, iterations, volume, proportion
            ),
        )
        ...
        plot_timeseries(
            attack,
            exp_data,
            num_lines=num_experiments,
            xlabel="Time (minutes)",
            ylabel=ylabel,
            name=name,
            series_labels=series_labels,
            save_legend=(m == "CongestionLossVsIterations"),
        )
...
def get_candidate_links(
    network, circuits, candidate_link_choice_method, attack_proportion=""
) -> list:
    ...
    for circuit_tag in circuit_tags:
        metric_path = path.join(
            data_root, iteration_tag, circuit_tag, metric_tag
        )
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        iterations = 3
        post_proc_timeseries(
            time_series_files, net, iterations, series_labels=["ECMP", "MCF"]
        )
