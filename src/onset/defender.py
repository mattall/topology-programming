...
class Defender:
    def __init__(
        ...
        network: str,
        bandwidth: str,
        candidate_link_choice_method: str,
        use_heuristic: str,
        tag="",
        attack_proportion="",
    ) -> None:
        ...
        self.tag = (
            tag if not attack_proportion else tag + "_" + attack_proportion
        )
        self.candidate_links = get_candidate_links(
            network, bandwidth, candidate_link_choice_method, attack_proportion
        )
        self.cached_sims_root = self.edge_congestion_file = path.join(
            SCRIPT_HOME,
            "data",
            "results",
            "{}_add_circuit_{}_{}_{}".format(
                self.network,
    ...
    def get_circuits_that_reduce_congestion_on_uv(self, u, v) -> dict:
        ...
        circuit_tags = [
            tag
            for tag in listdir(self.cached_sims_root)
            if circuit_regex.match(tag)
        ]
        for circuit_tag in circuit_tags:
            congestion_file = path.join(
                self.cached_sims_root, circuit_tag, metric_tag
            )
    ...
    def get_strategic_circuit(self):
        if self.candidate_link_choice_method == "heuristic":
            if (
                self.use_heuristic == "1"
            ):  # Link that reduces the congestion on most congested edge
                ...
                try:
                    ...
                    return
                logger.debug(
                    "Searching for circuits to relieve congestion on link: {}".format(
                        self.most_used_edge
                    )
                )
                candidate_links = (
                    self.get_circuits_that_reduce_congestion_on_uv(
                        *self.most_used_edge
                    )
                )
                try:
            ...
            if (
                self.use_heuristic == "2"
            ):  # adds most possible paths to target link - statically chosen
                (
                    self.edge_use,
                    self.edge_flows,
                    self.most_used_edge,
                ) = find_target_link(self.path_file)
                logger.debug(
                    "Searching for circuits to relieve congestion on link: {}".format(
                        self.most_used_edge
                    )
                ...
                try:
                    ...
                    return
                link_to_add = (
                    self.candidate_links[most_churnfull_id][1],
                    self.candidate_links[most_churnfull_id][2],
                )
            ...
            if (
                self.use_heuristic == "3"
            ):  # removes most possible paths from target link - statically chosen
                (
                    self.edge_use,
                    self.edge_flows,
                    self.most_used_edge,
            ...
            if (
                self.use_heuristic == "4"
            ):  # Link that introduces that greatest number of new paths
            ...
            if (
                self.use_heuristic == "5"
            ):  # Link that removes that greatest number of new paths
                candidate_links = self.get_circuits_that_change_total_paths(
                    positive=False
                )
                try:
            ...
            if (
                self.use_heuristic == "6"
            ):  # Introduce the new paths while adding greatest number of paths to a targeted link.
                edge_congestion = self.get_edge_congestion(
                    self.edge_congestion_file
                )
                try:
                ...
                for candidate in self.candidate_links:
                    ...
                    churn[candidate_circuit] = self.link_churn_on_path(
                        candidate_circuit, self.most_used_edge
                    )

                path_additive_links = (
                    self.get_circuits_that_change_total_paths(positive=True)
                )
                while churn:
            ...
            if (
                self.use_heuristic == "7"
            ):  # Introduce the new paths while removing greatest number of paths to a targeted link.
                edge_congestion = self.get_edge_congestion(
                    self.edge_congestion_file
                )
                try:
                ...
                if path_additive_links:
                    ...
                    return (
                        int(congestion_reducing_link[1]),
    ...
    def link_churn_on_path(self, circuit: tuple, link: tuple):
        ...
        path_diff_file = path.join(
            self.cached_sims_root,
            "path_diff",
            "{}_{}.txt".format(circuit[0], circuit[1]),
        )
