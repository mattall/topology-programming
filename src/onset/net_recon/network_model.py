...
class Network():
    ...
    def __init__(self, graph_file:str, output_file="", prefix=24):
        ...
        self.node_types = ["Amplifier", "ROADM", "Transponder", "Router", "Virtual", "Client"]
        self.link_types = ["Span", "Fiber", "Optical", "Layer3", "Virtual", "Client"]
    ...
    def init_client_links(self):
        ...
        for node in self.get_router_nodes():
            ...
            client_link_attr = {
                ...
                "fdL": defaultdict(float) # Flow density of link {(src, dest): fd}. Set when calculate_fdL is called.
            }
    ...
    def random_ipv4_address_space(self):
        ...
        address_space = ipaddress.IPv4Network(f'{ip_addr}/{self.prefix}', strict=False) # setting strict=False will mask with the given prefix.
    ...
    def remove_available_interface(self, interface_addr):
        ...
        try:
            ...
            print(e)
            print(type(interface_addr))
            print(f'Unable to remove interface {interface_addr}. \
    ...
    def import_graph(self, file_path):
        ...
        Reads inputed graph and add apporpiate attributes to it to describe a physical and logic network.
        """
        if file_path.endswith(".gml"):
            ...
            raise BaseException(f"Expected file of type .gml or .json. Got: {file_path}")
        
        nx.relabel_nodes(G, {n: str(i) for n, i in zip(G.nodes(), range(len(G.nodes())))}, copy=False)
        ...
        for i in range(fixed):
            for node in nodes:
                if nodes[node]['node_type'] == "Router":
                    node_tf = current_fdN[node] // (fixed+1) # plus 1 because we are counting real node as well.
                    # print(node, node_tf)
        ...
        for i in range(fixed):
            for u, v in links:
                ...
                if links[(u, v)]['link_type'] == 'Layer3':
                    u_v = str(link)
                    v_u = str((v, u))
                    link_u_v_tf = current_fdL[link][u_v] // (fixed+1) # plus 1 because we are counting real link as well.
                    link_v_u_tf = current_fdL[link][v_u] // (fixed+1) # plus 1 because we are counting real link as well.
                    # links[(u, v)]['fdL']
    ...
    def get_connected_virtual_node(self, real_node, virtual_node):
        ...
        connected_vn = next((vn for vn in prev_virtual_nodes if G.has_edge(vn, virtual_node)), None)
        return connected_vn
    ...
    def get_router_fdN(self):
        ...
        try:
            router_fdN.update({node : nodes[node]['fdN'] for node in nodes if nodes[node]['node_type'] == 'Router'})
            return router_fdN
        except:
            return router_fdN
    ...
    def get_virtual_fdN(self):
        ...
        try:
            virtual_fdN.update({node : G.nodes[node]['fdN'] for node in nodes if G.nodes[node]['node_type'] == 'Virtual'})
            return virtual_fdN
        except:
            return virtual_fdN
    ...
    def get_L3_fdL(self):
        ...
        try:
            l3_fdL.update({(u, v) : G[u][v]['fdL'] for u,v in links if G[u][v]['link_type'] == 'Layer3'})
            return l3_fdL
        except:
            return l3_fdL
    ...
    def get_virtual_fdL(self):
        ...
        try:
            virtual_fdL.update({(u, v) : G[u][v]['fdL'] for u,v in links if G[u][v]['link_type'] == 'Virtual'})
            return virtual_fdL
        except Exception as e:
            print(e)
            return virtual_fdL
...
def main(argv):
    ...
    # flows = [(0, 5, 100), (5, 0, 200)] # for testing
    # fdL = network.calculate_fdL(flows)
    # fdN = network.calculate_fdN()
    # print(f'fdL: {fdL}, fdN: {fdN}')
    
    # fixed = 2
    # network.create_virtual_nodes(fixed)
    # network.create_virtual_links(fixed)
