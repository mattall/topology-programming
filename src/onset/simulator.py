...
from onset.utilities.post_process import read_link_congestion_to_dict, read_result_val
...
class Simulation():
    def __init__(self, network_name: str,
        ...
        if self.use_heuristic.isdigit():
            self.EXPERIMENT_ID = "_".join([network_name, test_name, str(fallow_transponders), self.attack_proportion, self.te_method]).replace(
                "heuristic", "heuristic_{}".format(self.use_heuristic))
        else:
            self.EXPERIMENT_ID = "_".join(
                [network_name, test_name, str(fallow_transponders), self.attack_proportion, self.te_method])

        # Set Experiment absolute path
        if self.net_dir:
            self.EXPERIMENT_ABSOLUTE_PATH = path.join(
                SCRIPT_HOME, "data", "results", self.network_name, self.EXPERIMENT_ID)
    ...
    def evaluate_performance_from_adding_link(self, circuits_to_add=1):
        ...
        INITIAL_GRAPH_PATH = path.join(GRAPHS_PATH, self.network_name + "_0.dot")
    ...
    def perform_sim(self, circuits=1, start_iter=0, end_iter=0, repeat=False):
        ...
        with open(traffic, 'rb') as fob:
            ...
            for i in range(1, iterations+2):
                if self.shakeroute:
                    ITERATION_ID = "{}".format(self.topology_programming_method)
                ...
                try:
                    ...
                    elif self.topology_programming_method == 'cache' and sig_add_circuits and not circuits_added:
                        defender = Defender(
                            self.network_name, circuits, self.candidate_link_choice_method, self.use_heuristic, PREV_ITER_ABS_PATH, self.attack_proportion)
                    ...
                    elif self.topology_programming_method == 'onset' and sig_add_circuits:
                        optimizer = Link_optimization(
                            G=self.wolf.logical_graph, BUDGET=3, demand_matrix=self.nonce, network=self.network_name)
                        # optimizer.run_model()
                    ...
                    elif self.topology_programming_method == "greylambda" and sig_add_circuits:
                    ...
                    return_data["Strategy"].append(
                        "{} {}".format(self.te_method, self.topology_programming_method))
                    return_data["Routing"].append(
                        "{}".format(self.te_method).strip("-").upper())
                    return_data["Defense"].append("{}".format(self.topology_programming_method))
    ...
    def validate_yates_params(self):
        ...
        def verify_topo():
            ...
            if self.shakeroute and self.topology_programming_method != 'baseline':
                base_topo_file = path.join(
                    SCRIPT_HOME, "data", "graphs", "fiber_cut", self.shakeroute, self.topology_programming_method+".gml")
