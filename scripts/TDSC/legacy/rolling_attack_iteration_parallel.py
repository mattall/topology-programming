...
def main(argv):
    ...
    attack_sim = Attack_Sim(network, 
                            "rolling_attack_{}_{:03d}_{:03d}".format(strategy, start_iter, end_iter),
                            iterations=720, 
                            te_method=te_method,
                            method="optimal",
    ...
    result = attack_sim.perform_sim(circuits=10, start_iter=start_iter, end_iter=end_iter)
    for key in result:
        if key in data:
    ...
    with open("./data/results/{}_{}_{}_rolling_mixed_type_attack_{:03d}-{:03d}.csv".format(network,
                                                                                           te_method,
                                                                                           strategy,
                                                                                           start_iter,
                                                                                           end_iter), 'w') as fob:
